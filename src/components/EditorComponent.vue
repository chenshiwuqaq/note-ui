  ...
  <div style="display: flex">
    <div class="left-tree">
      ...
      <div class="file-icon-box">
        ...
        <el-icon class="file-icon" @click="showSearchInput = !showSearchInput">
          <Search />
        </el-icon>
        <el-icon class="file-icon">
        ...
        <template #default="{ data }">
          <div style="display: flex; align-items: center; width: 100%">
            ...
            <div class="node-function">
              <span class="icons">
<span
  v-if="data.isCollected === 0"
  class="iconfont icon-shoucang1"
  @click.stop="toggleCollect(data)"
  title="未收藏"
></span>
<span
  v-else
  class="iconfont icon-Filled_Star"
  @click.stop="toggleCollect(data)"
  title="收藏"
></span>
...
const toggleCollect = async (Node: TreeNode) => {
  try {
    const res = await axios.post("http://localhost:8003/editor/NodeChangeIsCollected", {}, {
      params: { nodeId: Node.id }
    });
    if (res.data.data === 0 || res.data.data === 1) {
      Node.isCollected = res.data.data;
      ElMessage.success("收藏状态改变");
    }
  } catch (error) {
...
const startAutoSave = () => {
  ...
  autoSaveIntervalId.value = window.setInterval(() => {
    // 必须同时满足「编辑器激活 + 选中有效节点」才执行保存
    if (isEditorActive.value && selectedNode.value && !selectedNode.value.children) {
...
.file-icon-box .el-icon:last-child .file-icon {
  margin-right: 0;
  margin-left: auto;
}
